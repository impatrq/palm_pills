...
def start(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        ...
        'hombre o mujer o otro?',
        reply_markup=ReplyKeyboardMarkup(
            reply_keyboard, one_time_keyboard=True, input_field_placeholder='Hombre o Mujer o Otro?'
        ),
    )
...
def gender(update: Update, context: CallbackContext) -> int:
    ...
    datos_db["genero"] = paciente_genero
...
def mensaje_siguiente (update: Update, context: CallbackContext) -> int:    
    alarma_uno = update.message.text
    datos_db["alarmauno"] = alarma_uno
    update.message.reply_text('Perfecto!. Si quiere programar la siguiente alarmas pulse /alarma_2, sino /cancel para finalizar')
    reply_markup=ReplyKeyboardRemove(),
    return HORA_2
...
def alarma_uno (update: Update, context: CallbackContext) ->int:
    sql = f"SELECT alarmauno from pacientes WHERE id = {7}"
    ...
    update.message.reply_text(str(respuesta[0][0]))

    return ConversationHandler.END

def alarma_dos (update: Update, context: CallbackContext) ->int:
    sql = f"SELECT alarmados from pacientes WHERE id = {7}"
...
def chau (update: Update, context: CallbackContext) ->int:
    alarma_dos = update.message.text
    datos_db["alarmados"] = alarma_dos
    ...
    cursor = con.cursor()
    print (datos_db)
    sql = f"SELECT * FROM pacientes WHERE id = {7}"
    cursor.execute(sql)
    respuesta = cursor.fetchall()
    if any(respuesta):
        try:
            sql = f"UPDATE pacientes SET alarmauno = '{datos_db['alarmauno']}', alarmados = '{datos_db['alarmados']}' WHERE id = {7}" 
            cursor.execute(sql)
            con.commit()
        except:
            print (datos_db)
    else: 
        sql = f"INSERT INTO pacientes (id, sexo, na, edad, alarmauno, alarmados) VALUES({7}, '{datos_db['genero']}', '{datos_db['na']}', '{datos_db['edad']}', '{datos_db['alarmauno']}', '{datos_db['alarmados']}')"
        cursor.execute(sql)
        con.commit()
        print (datos_db)
...
def main() -> None:
    ...
    conv_handler_alarma_uno = ConversationHandler (
        entry_points = [CommandHandler('alarma_1', alarma_uno)],
        states = { 0 : [ None ], },
        fallbacks = [ None ],
    )

    conv_handler_alarma_dos = ConversationHandler (
        entry_points = [CommandHandler('alarma_2', alarma_dos)],
        states = { 0 : [ None ], },
        fallbacks = [ None ],
    )
    ...
    dispatcher.add_handler(conv_handler_alarma_uno)
    dispatcher.add_handler(conv_handler_alarma_dos)
    # Start the Bot
